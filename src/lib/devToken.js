...
//
// Signs a JWT with the API secret from EXPO_PUBLIC_GETSTREAM_SECRET.
...
const DEV_ONLY_SECRET = process.env.EXPO_PUBLIC_GETSTREAM_SECRET || '';
...
export function createDevToken(userId, secret) {
  ...
  if (!signingSecret) {
    throw new Error(
      'GetStream secret not configured. Set EXPO_PUBLIC_GETSTREAM_SECRET in your .env file.'
    );
  }
